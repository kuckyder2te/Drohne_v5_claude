...
- **The `*_FMT` macros `sprintf` into one shared `logBuf`** declared in `myLogger.h` and defined in `src/myLogger.cpp` — plus once per tool, since `build_src_filter` excludes `myLogger.cpp` from tool builds while `lib/` (which uses the macros) is linked everywhere. All definitions must keep the same size as the declaration. `sprintf` is unbounded: the longest line in the project is `FlightController::logStatus()` at ~122 chars (it carries both ultrasonic raw values), which is why the buffer is 160. A longer format string needs the buffer grown with it.
- **The `*_FMT` macros expand to *two* statements** (`sprintf(...);LOGGER_NOTICE(logBuf)`) with no `do { … } while (0)` wrapper. An unbraced `if (x) LOGGER_NOTICE_FMT(…); else …` therefore fails to compile with `'else' without a previous 'if'`, and an unbraced `if` without `else` would silently log unconditionally. Always brace the branch.
...
2. **Sensor update auf festen Kadenzen** — Ultraschall alle `ULTRA_UPDATE_MS` (50 ms) für *einen* der beiden Sensoren im Wechsel (siehe Ultraschall-Abschnitt), Barometer alle `BARO_UPDATE_MS` (200 ms, aber nur mit `BARO_ENABLED`), Batterie ungetaktet. Vorher lief beides in *jedem* Durchlauf, was den Kern auf ~9 Hz drückte. Der IMU wird hier gar nicht mehr angefasst — er gehört Kern 1.
...
| [lib/Ultrasonic/Ultrasonic.cpp](lib/Ultrasonic/Ultrasonic.cpp) | **Zwei** HC-SR04, beide nach unten (S1 GP8/6, S2 GP9/7); gültiger Bereich ~2–300 cm. Jeder `update()` misst genau **einen** Sensor im Wechsel; die fusionierte Höhe ist das **Minimum** der frischen Kanäle. Einzige Höhenquelle, solange `BARO_ENABLED` aus ist — siehe Ultraschall-Abschnitt unten |
...
| `getDistance` | beide Ultraschallsensoren einzeln + fusionierter Wert + `spread` als JSON |
...
### Ultraschall: zwei Sensoren im Wechsel, Fusion per Minimum

Es hängen **zwei** HC-SR04 am Board, beide nach unten gerichtet (S1 an `PIN_ULTRASONIC_TRIG1/ECHO1` = GP8/GP6, S2 an `TRIG2/ECHO2` = GP9/GP7). Da `BARO_ENABLED` aus ist, ist dieses Paar die *einzige* Höhenquelle — für den Höhen-PID, das Liftoff-Gate (`LIFTOFF_HEIGHT_CM`) und den Höhensprung-Check in `checkSafety()`.

`Ultrasonic::update()` misst **genau einen** Sensor pro Aufruf und wechselt dann. Der Grund ist nicht die Schalllaufzeit, sondern die Blockade auf Kern 0: `pulseIn` wartet bis zu 25 ms (Timeout für ~4 m — hin *und* zurück sind das 8 m bzw. 23,3 ms, nicht 11,6 ms). Beide Sensoren in einem Aufruf wären bis zu 50 ms am Stück, und der Aufruf liegt in `NormalMode::loop()` zwischen CLI und Höhenregler. Beim Wechsel bleibt die Blockade pro Aufruf, wie sie war; jeder einzelne Sensor wird alle 100 ms abgetastet (Datenblatt verlangt ≥ 60 ms), die **fusionierte** Höhe aber weiterhin alle 50 ms — deshalb bleibt `PID_INTERVAL_MS` = 50 wirksam.

Fünf Dinge sind hier tragend:

- **Jeder Kanal hat seinen eigenen Filter-Ringpuffer** (`Chan::filterBuf`). Ein gemeinsamer Puffer — der frühere Einzelsensor-Stand — würde die Messwerte zweier verschieden montierter Sensoren zu einem Mittelwert verrühren, den es real nirgends gibt. Das ist der wahrscheinlichste Fehler beim Anfassen dieses Moduls.
- **Fusion ist das Minimum**, nicht der Mittelwert: über unebenem Boden oder einer Tischkante hält die Drohne damit Abstand zum *nächstliegenden* Hindernis, statt darüber abzusinken. Ein Mittelwert läge auf einer Höhe, die unter dem Gerät nirgends existiert.
- **Die Frischegrenze `ULTRASONIC_STALE_MS` (250 ms) ist beim Minimum sicherheitsrelevant, nicht kosmetisch.** Ohne sie würde ein ausgefallener Sensor seinen letzten — womöglich kleinen — Wert für immer im Minimum halten, und der Höhenregler stiege unbegrenzt dagegen an. 250 ms ≈ 2,5 Wechselperioden: zwei ausgefallene Pings werden überbrückt, ein toter Sensor fällt raus.
- **Es gibt bewusst keinen automatischen Ausreißer-Ausschluss.** Bei zwei Sensoren ist nicht entscheidbar, welcher lügt. Stattdessen wird die Abweichung sichtbar gemacht: `getDistance` liefert beide Rohwerte plus `spread`, und `logStatus()` zeigt sie als `U: <s1>/<s2>` mit. Sieht ein Sensor dauerhaft etwas Eigenes (Landebein, Kabel, Propellerschutz), zieht er das Minimum fest nach unten — **vor dem Armen mit `getDistance` prüfen, `spread` muss bei flach liegendem Gerät klein sein.**
- **Ist kein Kanal frisch, bleibt `_altitudeCm` stehen** und nur `isValid()` wird false. Das ist exakt das Verhalten des Einzelsensor-Stands und darf nicht "aufgeräumt" werden: `FlightController::updateControlLoop()` liest ohne `BARO_ENABLED` `getAltitudeCm()` **ohne** `isValid()`-Prüfung und verlässt sich darauf, dass der letzte gute Wert einen kurzen Aussetzer überbrückt.

Der Mindestabstand zwischen zwei Pings (`ULTRASONIC_MIN_PING_MS`, 40 ms) liegt als Sperre **in der Klasse**. Früher stand dafür ein `delay(30)` am Ende von `update()`, das Kern 0 blockierte, obwohl `NormalMode::loop()` ohnehin nur alle 50 ms misst. Als Sperre kostet es nichts und wirkt auch für Aufrufer ohne eigene Kadenz — `src/tools/test_ultrasonic/` hat genau deshalb jetzt eine eigene `ULTRA_UPDATE_MS`-Kadenz statt eines Pings pro Schleifendurchlauf.

Zeigt sich Übersprechen zwischen den Sensoren, ist `ULTRA_UPDATE_MS` (dann 60 ms → 120 ms je Sensor) die erste Stellschraube, nicht der Treiber.
...
- `src/tools/test_ultrasonic/` — beide HC-SR04 einzeln + fusionierter Wert + `spread`, Ausgabe alle 200 ms, gemessen auf `ULTRA_UPDATE_MS`-Kadenz. Das Werkzeug für die Montageprüfung: Papier vor *einen* Sensor halten — nur dessen Wert darf springen
...
- **Zwei Ultraschallsensoren im Wechsel statt eines** — siehe eigenen Abschnitt unten. Kurz: abwechselnd messen hält die Blockade auf Kern 0 bei den bisherigen ≤ 25 ms statt sie zu verdoppeln, und die fusionierte Höhe bleibt trotzdem auf 50 ms.
...
- **Drohne steigt beim Höhenhalten immer weiter / hält zu hoch**: `getDistance` prüfen. Fusioniert wird das **Minimum** beider Ultraschallsensoren — sieht einer dauerhaft etwas Nahes (Landebein, Kabel, Propellerschutz), zieht er die gemessene Höhe fest nach unten und der Regler steigt dagegen an. `spread` muss bei flach liegendem Gerät klein sein (< 3 cm).
- **`ok2` in `getDistance` dauerhaft 0**: Sensor 2 liefert seit > `ULTRASONIC_STALE_MS` nichts Gültiges — Verkabelung GP9/GP7 prüfen. Die Höhe kommt dann allein von Sensor 1, der Flug bleibt möglich, die Redundanz ist aber weg. Das Bootlog zeigt für beide Sensoren einen Probe-Ping.
