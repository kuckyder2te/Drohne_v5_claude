...
The firmware implements a **cascaded PID altitude + attitude stabilizer** for a quadcopter running on RP2040. `src/main.cpp` itself only declares the shared sensor/comm objects and delegates to a `Mode` (see [include/control/Mode.h](include/control/Mode.h)): a tiny interface (`setup()`/`loop()`) selected once at compile time via a global `Mode* activeMode`, analogous to the existing `comm` pointer pattern. `NormalMode` ([src/mode/NormalMode.cpp](src/mode/NormalMode.cpp)) implements real flight operation (`NORMALBETRIEB`); `KeyboardTestMode` ([src/mode/KeyboardTestMode.cpp](src/mode/KeyboardTestMode.cpp)) implements the in-firmware `TEST_KEYBOARD` mode; the two are mutually exclusive via `config.h`, and `main.cpp` never needs editing to add another one — a future test mode is just a new `Mode` subclass plus one line in `main.cpp` picking it. The actual flight state and control logic (arm/disarm, safety checks, PID + motor mixing, status log) lives in [src/control/FlightController.cpp](src/control/FlightController.cpp), called from `NormalMode`. The other six hardware test tools are separate standalone programs under `test/` (see Test Modes below) — they never touch `main.cpp`.

### Control Loop (normal operation, `NORMALBETRIEB` defined, i.e. `NormalMode` active)
...
| [src/control/InputHandler.cpp](src/control/InputHandler.cpp) | Normal-mode key/command handling (ARM timeout, `switch(key)`), called from `NormalMode::loop()`; translates key events into calls on `FlightController` |
| [include/control/Mode.h](include/control/Mode.h) | Tiny `setup()`/`loop()` interface implemented by every in-firmware mode; `main.cpp` holds a single `Mode* activeMode`, chosen at compile time |
| [src/mode/NormalMode.cpp](src/mode/NormalMode.cpp) | `Mode` implementation for real flight operation (`NORMALBETRIEB`): sensor updates, `FlightController::checkSafety()`, `InputHandler::handle()`, `FlightController::updateControlLoop()`/`logStatus()` |
| [src/mode/KeyboardTestMode.cpp](src/mode/KeyboardTestMode.cpp) | `Mode` implementation for the `TEST_KEYBOARD` in-firmware test mode (the other six moved to standalone tools under `test/`, see below) |

`MotorMixer`, `IMU`, `Barometer`, `Ultrasonic`, `Battery` live in `lib/<Name>/` (PlatformIO private libraries) rather than `src/`/`include/`, specifically so the standalone test tools under `test/` can link each one individually without pulling in `main.cpp` or unrelated modules — PlatformIO auto-links `lib/` into every environment and every `pio test` build regardless of `src_dir`/`test_build_src`. `PIDController`, `FlightController`, `CommChannel`, `Settings`, `InputHandler`, `Mode`/`NormalMode`/`KeyboardTestMode` stay in `src/`/`include/` since only the main firmware needs them. `main.cpp` itself now only declares the top-level sensor/comm objects (`baro`, `battery`, `ultrasonic`, `imu`, `comm`, `settings`), the `flightController` instance, and the one `activeMode` selection — adding a future in-firmware test mode means writing a new `Mode` subclass and pointing `activeMode` at it, without otherwise touching `main.cpp`.
...
`TEST_KEYBOARD` (BT/keyboard command echo + barometer print, implemented by `KeyboardTestMode`) is the **only** remaining `config.h`-gated in-firmware test mode — defined in [include/config.h](include/config.h), gated by the `NORMALBETRIEB` master switch (see Build System above): comment out `NORMALBETRIEB` and uncomment `TEST_KEYBOARD` to run it (this flips which concrete `Mode` `main.cpp`'s `activeMode` points at). It stayed in-firmware rather than becoming a standalone tool because it exercises `CommChannel::processCommand()` against real `PIDController`/`Settings` instances (via `FlightController`) — essentially the whole input/tuning stack, not an isolable hardware driver.
